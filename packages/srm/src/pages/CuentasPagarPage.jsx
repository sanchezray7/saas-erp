...
import { obtenerAgingAP } from '@saas/accounting'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="card">
        <div className="page-header" style={{ marginBottom: 12 }}>
          <h3 style={{ fontSize: '0.9rem', fontWeight: 700, margin: 0 }}>📋 Facturas pendientes por proveedor</h3>
                ...
                {porProveedor.map((p) => (
                  <tr key={p.proveedor_id}>
                    <td style={{ fontWeight: 600 }}>
                      <Link to={`/proveedores/${p.proveedor_id}`} className="link">
                        {p.proveedor_nombre || '—'}
                      </Link>
                    </td>
                    <td style={{ textAlign: 'right' }}>{p.facturas}</td>
                    <td style={{ textAlign: 'right', fontWeight: 600 }}>{formatMoney(p.total, moneda)}</td>
                    <td style={{ textAlign: 'right', color: '#dc2626', fontWeight: 600 }}>{formatMoney(p.vencido, moneda)}</td>
                    <td style={{ textAlign: 'right', color: '#059669' }}>{formatMoney(p.por_vencer, moneda)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
