...
function RecetaFormModal({ receta, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-lg)', maxWidth: 700, width: '100%', maxHeight: '90vh', overflow: 'auto', padding: 24 }}>
        ...
        <form onSubmit={handleSave}>
          ...
          <div style={{ marginTop: 16, marginBottom: 8 }}>
            ...
            {ingredientes.filter((i) => !i.es_subproducto).map((ing, i) => (
              <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 }}>
                <select value={ing.producto_id} onChange={(e) => updIng(ingredientes.indexOf(ing), 'producto_id', e.target.value)} style={{ flex: 1, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }} required>
                  <option value="">Seleccionar...</option>
                  {prodFiltered.map((p) => <option key={p.id} value={p.id}>{p.codigo ? `[${p.codigo}] ` : ''}{p.nombre}</option>)}
                </select>
                <input type="number" step="any" value={ing.cantidad} onChange={(e) => updIng(ingredientes.indexOf(ing), 'cantidad', Number(e.target.value))} placeholder="Cant." style={{ width: 80, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }} required />
                <select value={ing.unidad_medida} onChange={(e) => updIng(ingredientes.indexOf(ing), 'unidad_medida', e.target.value)} style={{ width: 70, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }}>
                ...
                <input type="number" step="any" value={ing.merma_porcentaje} onChange={(e) => updIng(ingredientes.indexOf(ing), 'merma_porcentaje', Number(e.target.value))} placeholder="Merma %" style={{ width: 70, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }} />
            ...
            <button type="button" onClick={() => addIngrediente(false)} style={{ fontSize: '0.82rem', color: 'var(--color-accent)', cursor: 'pointer', background: 'none', border: 'none', padding: 0 }}>+ Agregar ingrediente</button>
          </div>
          ...
          <div style={{ marginTop: 12, marginBottom: 16 }}>
            ...
            {ingredientes.filter((i) => i.es_subproducto).map((ing, i) => (
              <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 }}>
                <select value={ing.producto_id} onChange={(e) => updIng(ingredientes.indexOf(ing), 'producto_id', e.target.value)} style={{ flex: 1, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }}>
                  <option value="">Seleccionar...</option>
                  {prodFiltered.map((p) => <option key={p.id} value={p.id}>{p.codigo ? `[${p.codigo}] ` : ''}{p.nombre}</option>)}
                </select>
                <input type="number" step="any" value={ing.cantidad} onChange={(e) => updIng(ingredientes.indexOf(ing), 'cantidad', Number(e.target.value))} placeholder="Cant." style={{ width: 80, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }} />
                <select value={ing.unidad_medida} onChange={(e) => updIng(ingredientes.indexOf(ing), 'unidad_medida', e.target.value)} style={{ width: 70, padding: '6px 8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: '0.85rem' }}>
            ...
            <button type="button" onClick={() => addIngrediente(true)} style={{ fontSize: '0.82rem', color: 'var(--color-accent)', cursor: 'pointer', background: 'none', border: 'none', padding: 0 }}>+ Agregar subproducto</button>
          </div>
            ...
            <tbody>
              {recetas.map((r) => (
                <>
                  ...
                  {expanded === r.id && (
                    <tr key={`ing-${r.id}`}>
                      <td colSpan={6} style={{ padding: '8px 24px 16px', background: 'var(--color-surface-alt)' }}>
                        <div style={{ fontSize: '0.78rem' }}>
                          ...
                          {r.instrucciones && <p style={{ marginTop: 8, color: 'var(--color-text-muted)', fontStyle: 'italic' }}>📝 {r.instrucciones}</p>}
                        </div>
                      </td>
                    </tr>
                  )}
